...
console.log(bounds);
    ...
    //https://jetstream.gsi.ie/iwdds/servlet/controller?beta=false&amp;maxlat=</Source>
    //&amp;asset=iwdds&amp;event=DATA_TYPE_THEME</SourceEnd>
...
function openIWDDS(areaSelect){
	var iwddsURL ='';
	
				var bounds = areaSelect.getBounds();
								
				iwddsURL = 'https://jetstream.gsi.ie/iwdds/servlet/controller?beta=false&maxlat=' + bounds._northEast.lat.toFixed(6) + '&minlong='+ bounds._northEast.lng.toFixed(6) + '&maxlong='+ bounds._southWest.lng.toFixed(6) + '&minlat=' + bounds._southWest.lat.toFixed(6) +'&asset=iwdds&event=DATA_TYPE_THEME';
				
				//console.log(iwddsURL);
				window.open( iwddsURL, "_blank");
}

/* function listSurveys(areaSelect){
...
/* function listSurveys(areaSelect){
    ...
    "opacity": 0.65
		...
		filter: function (feature, layer){
			console.log(feature);
			return bounds.intersects(feature.geometry.coordinates);
		}
		}).addTo(map);
