...
map.getPane('overlayPane').style.zIndex = 200; */

var basemapPane = map.createPane('basemapPane');
map.getPane('basemapPane').style.zIndex = 2;

var OSIAdmiraltyPane = map.createPane('OSIAdmiraltyPane');
map.getPane('OSIAdmiraltyPane').style.zIndex = 10;
...
map.getPane('bathyContoursPane').style.zIndex = 6;

var bathyCoastalContoursPane = map.createPane('bathyCoastalContoursPane');
map.getPane('bathyCoastalContoursPane').style.zIndex = 7;
...
var OSI_Discovery = L.esri.dynamicMapLayer({
	...
	pane: 'OSIAdmiraltyPane',
	opacity : 0.75,
	attribution: '<a href="//www.osi.ie">Ordinance Survey Ireland</a>'	
}).addTo(map); 

var OSI_Basemap = L.esri.dynamicMapLayer({
	url: OSI_URL + 'basemap_public/MapServer',
	token : OSI_Key,
	f: 'json',
	client: 'referer',
	pane: 'OSIAdmiraltyPane',
...
//infomar data
 ...
 surveys.bindPopup(function (layer) {
	 //var area = Math.round(layer.feature.properties.Area_sqkm, 2);
...
var OSM_Coastal_bounds =[[51.154, -11.8], [56.4, -5.2]];

//Bathymetry & Contour Tiles
 var Overall_Bathy_Contours = L.tileLayer.fallback('//maps.marine.ie/INFOMAR_Tiles/Bathy_Contours/Overall/{z}/{x}/{y}.png', {
	maxZoom: 11,
	minZoom: 6,
 ...
 var Coast_Bathy_Contours = L.tileLayer.fallback('//maps.marine.ie/INFOMAR_Tiles/Bathy_Contours/Coastal/{z}/{x}/{y}.png', {
	maxZoom: 13,
	minZoom: 9,
	opacity : 0.7,
	errorTileURL: 'empty.png',
	pane: 'bathyCoastalContoursPane',
	bounds: OSM_Coastal_bounds,
	attribution: '<a href="http://www.infomar.ie">INFOMAR</a>'
}).addTo(map);

/* //SR & Contour Tiles
 var Overall_SR_Contours_NW = L.tileLayer('//maps.marine.ie/INFOMAR_Tiles/SR_Contours/Overall_NW/{z}/{x}/{y}.png', {
	maxZoom: 12,
	minZoom: 6,
	bounds: OSM_Overall_bounds,
	errorTileURL: 'empty.png',
 ...
 var Coast_SR_Contours_NW = L.tileLayer('//maps.marine.ie/INFOMAR_Tiles/SR_Contours/Coastal_NW/{z}/{x}/{y}.png', {
	maxZoom: 14,
	minZoom: 9,
	pane: 'srContoursPane',
	bounds: OSM_Coastal_bounds,
...
//SR_Only tiles
/* var Overall_SR_NW = new L.tileLayer('//maps.marine.ie/INFOMAR_Tiles/SR_Only/Overall_NW/{z}/{x}/{y}.png', {
		maxZoom: 11,
		minZoom: 6,
        noWrap: true,
		errorTileURL: 'empty.png',
...
var Overall_SR_NE = new L.tileLayer('//maps.marine.ie/INFOMAR_Tiles/SR_Only/Overall_NE/{z}/{x}/{y}.png', {
        maxZoom: 11,
		minZoom: 6,
		noWrap: true,
		errorTileURL: 'empty.png',
...
var Coast_SR_NW = new L.tileLayer('//maps.marine.ie/INFOMAR_Tiles/SR_Only/Coastal_NW/{z}/{x}/{y}.png', {
        maxZoom: 13,
		minZoom: 9,
		noWrap: true,
		errorTileURL: 'empty.png',
		...
		//opacity : 0.8,
		bounds: OSM_Coastal_bounds,
		pane: 'srCoastalContoursPane',
		attribution: '<a href="//infomar.ie">INFOMAR/INSS</a>'	
		});	

var Coast_SR_NE = new L.tileLayer('//maps.marine.ie/INFOMAR_Tiles/SR_Only/Coastal_NE//{z}/{x}/{y}.png', {
        maxZoom: 13,
		minZoom: 9,
		noWrap: true,
		errorTileURL: 'empty.png',
...
var polygonLgd = new L.layerGroup([npwsNHA, npwsPNHA, npwsSPA, npwsSAC, surveys, surveys2017]);
...
/* var roads250k = L.esri.featureLayer({
	url: '//services6.arcgis.com/uWTLlTypaM5QTKd2/ArcGIS/rest/services/Regional/FeatureServer/24',
	transparent: true,
	zIndex: 500,
	attribution: '<a href="//www.osi.ie">OSI</a>'	
});
...
var irishterritorialSea = L.esri.featureLayer({
	url: '//atlas.marine.ie/arcgis/rest/services/AdministrativeUnits/MapServer/4',
	transparent: true,
	zIndex: 500,
...
var irishDesignatedArea = L.esri.featureLayer({
	url: '//atlas.marine.ie/arcgis/rest/services/AdministrativeUnits/MapServer/11',
	transparent: true,
	zIndex: 500,
	attribution: '<a href="//www.dccae.gov.ie/en-ie/natural-resources/topics/Oil-Gas-Exploration-Production/Pages/home.aspx">PAD</a>'	
});
...
var irishMSFD = L.esri.featureLayer({
	url: 'http://atlas.marine.ie/arcgis/rest/services/ReportingUnits/MapServer/4',
	transparent: true,
	zIndex: 500,
	 attribution: '<a href="//www.marine.ie">Marine Institute</a>'		
});
			...
			var overlays = [
							{
								...
								layers    : { 
										...
										"OSI Basemap" : OSI_Basemap,
										"OSI Digital Globe" : OSI_DigitalGlobe,
									/* 	"OSI Main Harbours": mainHarbours,
										"OSI Other Harbours": otherHarbours	 */
										}	
                             },
							{
								...
								expanded : false,
								layers    : { 
								"Overall Bathymetry & Contours" : Overall_Bathy_Contours,
								"Coastal Bathymetry & Contours" : Coast_Bathy_Contours,
/* 								"Overall Shaded Relief NW": Overall_SR_NW,
								"Overall Shaded Relief NE": Overall_SR_NE,
								"Coastal Shaded Relief NW": Coast_SR_NW,
								"Coastal Shaded Relief NE": Coast_SR_NE, */
								"INFOMAR/INSS Surveys": surveys,
								"Planned 2017 Surveys" : surveys2017,
...
/* var mylayersControl = new L.control.layers(baseMap, overlays);
...
map.on('moveend', function(){
	if (map.getZoom() < 9) {
		...
		opacityLayer = Coast_Bathy_Contours;
		console.log(opacityLayer);
		opacitySlider.setOpacityLayer(opacityLayer);
	}
});
