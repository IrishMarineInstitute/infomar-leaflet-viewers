...
layer.on('mouseover', function(e){
	...
	function showChartImageWindow(bathymetry, folder, title, backscatter, shadedNW, shadedNE){
		...
		if ((info.browser != "firefox") && info.acrobat == false){
			...
			var imageIframe = "<div><p style=\"font-size: 16px; font-weight: strong; margin: 5px; color: #4A4A4A;\">"+ title +"<button id=\"btnCloseShipwreck\" onclick='closeShipwreckWindow()' style=\"float:right;\ class=\"ui-button-text-icon-primary ui-icon ui-icon-close\"><span class=\"ui-icon ui-icon-close\"></span></button></p></div><div id=\"chartimageDiv\"><embed id=\"chartimage\" src='" + baseURL + "/" + bathymetry + ".pdf'\" width=\"100%;\" height=\"100%;\" type=\"application/pdf\"></div><div><button id=\"bathymetry\" type=\"button\" class=\"btn btn-digital\" style=\"margin-top: 10px; margin-right: 10px; float:left;\" onclick='openNewImage(\"" + baseURL+ "\",\"" + bathymetry+ "\", bathymetry)'>Bathymetry</button><button id=\"backscatter\" type=\"button\" class=\"btn btn-digital\" style=\"margin-top: 10px; margin-right: 10px; float:left;\" onclick='openNewImage(\"" + baseURL+ "\",\"" + backscatter+ "\", backscatter)'>Backscatter</button><button id=\"shadedNE\" type=\"button\" class=\"btn btn-digital\" style=\"margin-top: 10px; margin-right: 10px; float:left;\" onclick='openNewImage(\"" + baseURL+ "\",\"" + shadedNE+ "\", shadedNE)'>Shaded Relief North East</button><button id=\"shadedNW\" type=\"button\" class=\"btn btn-digital\" style=\"margin-top: 10px; margin-right: 10px; float:left;\" onclick='openNewImage(\"" + baseURL+ "\",\"" + shadedNW+ "\", shadedNW)'>Shaded Relief North West</button><button type=\"button\" class=\"btn btn-digital\" style=\"margin-top: 10px; margin-right: 10px; float:left;\"class=\"ui-button-text-icon-primary\" onclick='openNewImageLink(fullPageLink)'><span class=\"ui-icon ui-icon-extlink\"></span></button></div>";
